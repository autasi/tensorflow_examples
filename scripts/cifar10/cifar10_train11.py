...
from arch.graph import cifar10_inception_v3
...
# trains CIFAR10 residual network using learning rate of exponential decay
def main():
    ...
    # auxiliary logit output required for optimization
    aux_logit = tuple_list_find(layers, "aux_fc")[1]
    
    # label smoothing
    # https://github.com/tensorflow/models/blob/master/research/inception/inception/slim/losses.py
    label_smoothing = 0.1
...
#    gt = gt * smooth_positives + smooth_negatives        
    ...
    aux_cross_entropy = tf.reduce_mean(tf.losses.softmax_cross_entropy(onehot_labels=gt, logits=aux_logit))
    aux_weight = 0.3
