...
from arch.graph import cifar10_bottleneck_densenet
...
def main():
    ...
    # create network
    layers, variables = cifar10_bottleneck_densenet(x)
    ...
    with session.as_default():
        ...
        net_path = os.path.join(cifar10_net_folder, "cifar10_bottleneck_densenet_expdecay.pkl")
        save_variables(session, net_path)
    session.close()
    session = None
#('Epoch: ', 39)
#('Learning rate: ', 0.00027542287033381673)
#('Test accuracy: ', 0.84121096)
#('Train accuracy: ', 0.9246946)
